...
TextBuffer::TextBuffer(int maxLines)
    : cursorRow(0), cursorCol(0), mode(VimMode::NORMAL),
      maxLines(maxLines), scrollOffset(0), visibleLines(20) {
...
void TextBuffer::ensureCursorVisible() {
    // Auto-scroll to keep cursor in view (works in all modes)
    // Skip auto-scroll in COPY mode since it has manual scrolling
    if (mode == VimMode::COPY) return;
    ...
    else if (cursorRow >= scrollOffset + visibleLines) {
        scrollOffset = cursorRow - visibleLines + 1;
    }

    // Keep scrollOffset in valid range
    scrollOffset = std::max(0, std::min(scrollOffset, std::max(0, (int)lines.size() - 1)));
}
